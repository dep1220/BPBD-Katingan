<x-app-layout>
    <x-slot name="header">
        <x-page-title>Tambah Dokumen</x-page-title>
    </x-slot>
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select id="kategori" name="kategori" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 @error('kategori') @enderror">
                                    ...
                                    @foreach($kategoris as $kategori)
                                        <option value="{{ $kategori['value'] }}" @selected(old('kategori') == $kategori['value'])>
                                            {{ $kategori['icon'] }} {{ $kategori['label'] }}
                                        </option>
                                    @endforeach
                                    <option value="custom" @selected(old('kategori') == 'custom')>➕ Kategori Custom</option>
                                </select>
                                ...
                                <div id="custom-kategori-input" class="mt-2" style="display: none;">
                                    <input type="text" id="kategori_custom" name="kategori_custom" value="{{ old('kategori_custom') }}"
                            ...
                            <div>
                                ...
                                <input type="file" name="file" id="file" required
                                       ...
                                       class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100 @error('file') border-red-500 @enderror">
                                @error('file')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                                <p class="mt-1 text-sm text-gray-500">File: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX. Maksimal: 5MB.</p>
                            </div>

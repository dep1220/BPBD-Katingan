    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-sm sm:text-base md:text-2xl text-orange-600 tracking-tight flex items-center truncate">
                <span class="truncate">{{ __('Edit User: ') }}{{ $user->name }}</span>
            </h2>
        </div>
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.users.update', $user) }}" method="POST">
                        ...
                        <div class="mb-6 border-t pt-6">
                            <h4 class="text-md font-semibold text-gray-700 mb-4">Ubah Password (Opsional)</h4>
                            ...
                            <div class="mb-6" x-data="{ showPassword: false }">
                                ...
                                <p class="mt-1 text-xs text-gray-500">Minimal 8 karakter jika ingin mengubah</p>
                                @error('password')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
